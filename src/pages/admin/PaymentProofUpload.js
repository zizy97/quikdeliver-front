...
const PaymentProofUploadPage = () => {
  ...
  const createPackageDeliveryRequest = (file, requestId, description) => {
    return packageService.paymentProofUpload(file, packageRequest.id, description);
  };

  // useEffect(async()=>{
  //   setLoading(true);
  //   try{
  //   const pack = await packageService.findPackageDeliveryRequestHash(requestId);
    ...
    <>
      ...
      >
        <Container maxWidth='sm'>
          {showModal?<ConfirmDialog title={'Successfull'} msg={'Payment proof upladed successfully.'} handleOk={()=>setShowModal(false)}/>:null}
          {showErrorModal?<ConfirmDialog title={'Error'} msg={errorMessage}/>:null}
          <Formik
            initialValues={{
              note: '',
              itemImages: []
            ...
            onSubmit={async (values, { resetForm }) => {
                ...
                try{
                    const res = await createPackageDeliveryRequest(files[0], requestId, values.note);
                    console.log(res);
                    setLoading(false);
                    setShowModal(true);
                    setFiles([]);
                ...
                <Table>
                  ...
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell>{parseInt(packageRequest?.distance)/1000}</TableCell>
                    </TableRow>
                    <TableRow>
                      <TableCell>Description</TableCell>
                      <TableCell>{packageRequest?.packageDescription}</TableCell>
                    </TableRow>
                    <TableRow>
...
export default class PaymentProofUpload extends React.Component {
  constructor(props) {
    super(props);
  }
